...
class CampaignTemplate(APIView):
        ...
        try:
            if checkduptemplatename is False:
                if serializer.is_valid():
                    img_data =  base64.b64decode(request.data['body'])
                    imgext = request.data['title']
                    processedimage =  receiveImage(img_data,imgext)
                    serializer.save(body=processedimage['body'],preview_body=processedimage['preview'])
                    responseData ={
    ...
    def put(self,request,format=None):
            ...
            try:
                ...
                campaigntemplateupdate.body =processedimage['body']
...
def activeCampaign(activecampaign):
    for counter,element in enumerate(activecampaign):
        ...
        if campaigntype !='birthday':
           ...
           Campaignnotification = CampaignNotification(customerrecord,templatename,subject,merchantname,campaigntype,merchID)     
        else:
           merchantname = merchant.objects.filter(id=element['merchID']).values('serviceID').first()
           templatename=campaigntemplate.objects.filter(merchID=element['merchID']).filter(id=element['tempateID']).filter(active=True).values('title','body').first()
           customerrecord= list(customer.objects.filter(merchID=element['merchID']).filter(DOB=todaydate).values('firstname','lastname','emailaddress'))
           Campaignnotification = CampaignNotification(customerrecord,templatename,subject,merchantname,campaigntype,merchID)   
...
def CampaignNotification(customerrecord,templatename,subject,merchantname,campaigntype,merchID):
    for counter,element in enumerate(customerrecord):
        ...
        emailnotification =emailNotification(customerfullname,templatesample,customeremail,subject,merchantname,campaigntype,templatetitle,merchID)
...
def emailHistory(merchantname,subject,customeremail,campaigntype,templatetitle,merchID):
    c= campaignhistory(merchID=merchID,merchantname=merchantname['serviceID'],subject=subject,customeremail=customeremail,campaigntype=campaigntype,templatename=templatetitle)
    c.save()
   
def receiveImage(img_data,imgext):
    save_path_html = os.path.join(settings.BASE_DIR,'campaigntemplates',imgext+'.html')
    save_path_png = os.path.join(settings.BASE_DIR,'campaigntemplates',imgext+'.png')
    databasepath_html= 'campaigntemplates/'+imgext+'.html'
    databasepath_png= 'campaigntemplates/'+imgext+'.png'
    with open(save_path_html, "wb") as fh:
          fh.write(img_data)
    with open(save_path_png, "wb") as fh:
          fh.write(img_data)
    record={'body':databasepath_html,'preview':databasepath_png}
    return record
